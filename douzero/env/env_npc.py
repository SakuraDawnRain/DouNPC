...
from douzero.env.game_npc import GameEnv
...
def get_obs_npc(infoset, action):
    """
    This function obtains observations with imperfect information
    from the infoset. It has three branches since we encode
    different features for different positions.
    ...
    if infoset.player_position == 'landlord':
        ...
        return _get_obs_landlord_up_npc(infoset, action)
    elif infoset.player_position == 'landlord_down':
        return _get_obs_landlord_down_npc(infoset, action)
    else:
        raise ValueError('')
    
def _get_obs_landlord_up_npc(infoset, action):
    """
    Obttain the landlord_up features. See Table 5 in
    https://arxiv.org/pdf/2106.06135.pdf
    """
    ...
    my_action_batch = np.zeros(my_handcards_batch.shape)
    my_action_batch[0: i] = _cards2array(action)
...
def _get_obs_landlord_down_npc(infoset, action):
    """
    Obttain the landlord_down features. See Table 5 in
    https://arxiv.org/pdf/2106.06135.pdf
    """
